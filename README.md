...
| DMP biomass yield | **Ranking covariate only, never a yield.** Across 16 representative admin-scale frames the implied harvest index is inside the agronomic 0.30-0.55 band in only 5 (median 0.27, median over-prediction 1.7x), so the level is not usable. Its value is concentrated where CPI ranks BACKWARDS: 3 wins, 0 losses on those 8 products, and a dead heat where CPI already works. (The older "implied HI 0.02" came from targeted ASAL drought crop-cuts and was inadmissible; the conclusion now rests on representative data.) |
